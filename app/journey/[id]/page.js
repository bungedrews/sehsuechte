...
export default function JourneyPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const [{ data: sessionData }, { data, error }] = await Promise.all([
        ...
        supabase.from('scans')
          .select('scanned_at, artwork_id, artworks(id, title, artist, short, description, image_url)')
          .eq('session_id', id)
  ...
  if (notFound) {
    return (
      <main className="min-h-screen flex flex-col justify-center p-8 gap-4">
        <h1 className="t-heading">Journey not found</h1>
        <p className="t-body">This link may have expired or doesn't exist.</p>
      </main>
    )
  }
  ...
  return (
    <main className="min-h-screen flex flex-col">
      <div className="flex flex-col p-8 gap-4 mt-8">
        <div className="t-label">{sessionName ? `${sessionName}'s Journey` : 'A Journey'}</div>
        <h1 className="t-heading">
          {artworkCount} work{artworkCount !== 1 ? 's' : ''}<br />explored
        </h1>
      </div>
