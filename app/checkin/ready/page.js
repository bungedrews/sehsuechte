...
function ReadyContent() {
  ...
  // On mount: load all existing scans from Supabase
  useEffect(() => {
    ...
    // Detect if opened as a new tab from NFC scan
    if (window.opener) {
      setIsNewTab(true)
    }

    loadExistingScans(sessionId)
  }, [])

  // After existing scans load, handle any incoming ?code or ?exit param
  useEffect(() => {
    if (loading) return
  ...
  async function loadExistingScans(sessionId) {
  ...
  if (loading) {
    return (
      <main className="min-h-screen flex flex-col items-center justify-center gap-3" style={{ fontFamily: 'var(--font-mono)', background: '#f5f2eb' }}>
  ...
  return (
    <main className="min-h-screen flex flex-col justify-between p-8" style={{ fontFamily: 'var(--font-mono)', background: '#f5f2eb' }}>
      ...
      <div className="flex flex-col gap-8">
        ...
        {isNewTab && (
          <div className="flex items-center justify-between bg-neutral-100 rounded px-4 py-3">
            <p className="text-xs text-neutral-500">You can close your previous tab</p>
            <button onClick={() => setIsNewTab(false)} className="text-xs text-neutral-400 hover:text-neutral-700 transition-colors ml-4">✕</button>
          </div>
        )}
        ...
        {status && <p className="text-xs text-neutral-400 tracking-wide">{status}</p>}
